...
class FiskalizacijaV1Adapter(ProviderAdapter):
    ...
    def _calculate_security_code(self, fiscal_data, dat_vrijeme=None):
        """Calculate ZastKod (security code) per Croatian fiscalization spec.
        
        According to Technical Specification for Fiscalization:
        1. Concatenate: OIB + DatVrijeme + BrOznRac + OznPosPr + OznNapUr + IznosUkupno
        2. Sign the concatenated string with RSA-SHA1 using the private key
        3. MD5 hash the signature bytes to produce the ZKI
        """
        import hashlib
        ...
        # Use provided datetime or generate new one
        if dat_vrijeme is None:
        ...
        # In sandbox mode or without private key, return a placeholder ZKI
        key_path = self.cert_meta.get('key_path') if self.cert_meta else None
        if self.mode == 'sandbox' or not key_path:
            # Return MD5 of concatenated string as fallback for sandbox
        ...
        try:
            ...
            password = self.cert_meta.get('password')
            if password:
            ...
            # Sign with RSA-SHA1 (PKCS#1 v1.5 padding)
            ...
            # MD5 of the signature bytes gives the ZKI
            zki = hashlib.md5(signature).hexdigest()
            return zki
            
        except Exception as e:
            logger.error(f'Failed to calculate ZKI: {e}')
    ...
    def sign_payload(self, payload):
        ...
        # Create signature template - MUST use RSA_SHA1 per Croatian fiscalization spec
        signature_node = xmlsec.template.create(root, xmlsec.Transform.EXCL_C14N, xmlsec.Transform.RSA_SHA1)
        root.append(signature_node)
        
        # Add reference with URI to the RacunZahtjev element - SHA1 digest
        uri_id = root.get('Id')
        ref = xmlsec.template.add_reference(signature_node, xmlsec.Transform.SHA1, uri=f"#{uri_id}")
